...
class _ProfilePageState extends State<ProfilePage> {
  int currentIndex = GlobalUserInfo.overallIndex; //get this info from server
  double _itemExtent = 250;
  final CarouselController _controller = CarouselController();
  ...
  String currentGoogleId = GlobalUserInfo.googleId;
  String biography = '';
  String kmName = '';
  String university = '';
  ...
  Future<void> _loadUserProfile() async {
    final response = await http.get(
      Uri.parse('$baseUrl?query=$currentGoogleId '),
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        kmName = retrievedUser.kmName;
        university = retrievedUser.university;
        currentUserName = retrievedUser.firstName + retrievedUser.kmName + retrievedUser.surName;

        biographyFieldController.text = retrievedUser.biography;
      });
    } else {
      print('Kunde inte ladda profil');
    }
  }
  ...
  Future<void> _updateUserProfile() async {
    ...
    currentUser!.biography = biographyFieldController.text;

    final response = await http.put(
      Uri.parse('http://10.0.2.2:8080/api/user/update/\${currentUser!.googleId}'),
      headers: {'Content-Type': 'application/json'},
    ...
    if (response.statusCode == 200) {
      ...
      print('Misslyckades med uppdatering: \${response.statusCode}');
    }
  }
  ...
  void _updateCurrentIndex() {
    if (_controller.hasClients) {
      ...
      if (position.hasPixels) {
        ...
        setState(() {
          currentIndex = index;
          GlobalUserInfo.overallIndex =index;
        });
      }
    }
  }
  ...
  alignment: Alignment.center,
  child: Text(kmName, style: TextStyle(color: Color.fromARGB(255, 210, 210, 210), fontSize: 16, fontFamily: 'InknutAntiqua')), //BACKEND userName-värde
  ),
  ...
  alignment: Alignment.center,
  child: Text(university, style: TextStyle(color: Color.fromARGB(255, 210, 210, 210), fontSize: 16, fontFamily: 'InknutAntiqua')), //BACKEND userName-värde
  ),
